...
const AllInvoices = () => {
  const [invoices, setInvoices] = useState([]); // Fixed: plural name
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        setInvoices(invoicesData.sort((a, b) => new Date(b.invoiceDate) - new Date(a.invoiceDate)));
      } catch (err) {
  ...
  const filteredInvoices = useMemo(() => {
    // Ensure invoices is always treated as an array
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {filteredInvoices.map(invoice => (
                  <tr key={invoice._id} className="hover:bg-slate-50">
                    ...
                    <td onClick={() => navigate(`/invoices/${invoice._id}`)} className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 cursor-pointer">
                      {invoice.dueDate ? moment(invoice.dueDate).format('MMM D, YYYY') : 'N/A'}
                    </td>
